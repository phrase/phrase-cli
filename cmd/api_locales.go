...
func initLocaleDownload() {
	...
	var LocaleDownload = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if params.IsSet(helpers.ToSnakeCase("fallbackForUnverifiedTranslations")) {
				localVarOptionals.FallbackForUnverifiedTranslations = optional.NewBool(params.GetBool(helpers.ToSnakeCase("FallbackForUnverifiedTranslations")))
			}
	...
	AddFlag(LocaleDownload, "string", helpers.ToSnakeCase("IfModifiedSince"), "", "Last modified condition, see [Conditional GET requests / HTTP Caching](/en/api/strings/pagination#conditional-get-requests-%2F-http-caching) (optional)", false)
	AddFlag(LocaleDownload, "string", helpers.ToSnakeCase("IfNoneMatch"), "", "ETag condition, see [Conditional GET requests / HTTP Caching](/en/api/strings/pagination#conditional-get-requests-%2F-http-caching) (optional)", false)
	AddFlag(LocaleDownload, "string", helpers.ToSnakeCase("Branch"), "", "specify the branch to use", false)
	AddFlag(LocaleDownload, "string", helpers.ToSnakeCase("FileFormat"), "", "File format name. See the [format guide](https://support.phrase.com/hc/en-us/sections/6111343326364) for all supported file formats.", false)
	...
	AddFlag(LocaleDownload, "bool", helpers.ToSnakeCase("FilterByPrefix"), "", "Only download translation keys containing the prefix specified by `translation_key_prefix`, and remove that prefix from the generated file. Requires `translation_key_prefix` to be set.", false)
