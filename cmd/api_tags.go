...
func initTagsList() {
	...
	var TagsList = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if params.IsSet(helpers.ToSnakeCase("onlySystemTags")) {
				localVarOptionals.OnlySystemTags = optional.NewBool(params.GetBool(helpers.ToSnakeCase("OnlySystemTags")))
			}
	...
	AddFlag(TagsList, "bool", helpers.ToSnakeCase("ExcludeSystemTags"), "", "Excludes tags generated by the system, e.g. job, upload or figma tags. Mutually exclusive with `only_system_tags`.", false)
	AddFlag(TagsList, "bool", helpers.ToSnakeCase("OnlySystemTags"), "", "Returns only system-generated tags (e.g. job, upload, figma) and excludes user-defined tags. Mutually exclusive with `exclude_system_tags`.", false)
	AddFlag(TagsList, "string", helpers.ToSnakeCase("Branch"), "", "specify the branch to use", false)
	AddFlag(TagsList, "string", helpers.ToSnakeCase("Q"), "", "Specify a query to filter tags by name.", false)
