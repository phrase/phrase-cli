...
func initRepoSyncEventShow() {
	...
	var RepoSyncEventShow = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			data, api_response, err := client.RepoSyncsApi.RepoSyncEventShow(auth, accountId, repoSyncId, id, &localVarOptionals)
	...
	RepoSyncsApiCmd.AddCommand(RepoSyncEventShow)
...
func initRepoSyncEvents() {
	params := viper.New()
	var use string
	// this weird approach is due to mustache template limitations
	use = strings.Join(strings.Split("repo_sync/events", "/")[1:], "_")
	var RepoSyncEvents = &cobra.Command{
		Use:   use,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			client := api.NewAPIClient(cfg)
			localVarOptionals := api.RepoSyncEventsOpts{}
